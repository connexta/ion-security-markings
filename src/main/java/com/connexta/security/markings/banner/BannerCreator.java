...
public class BannerCreator {
  private static BannerCreator instance;
  ...
  // singleton - make the default constructor private
  private BannerCreator() {}

  public static BannerCreator getInstance() {
    if (instance == null) {
      instance = new BannerCreator();
    }
    return instance;
  }
  ...
  private void appendClassification(StringBuilder builder, ISM ism) {
    ...
    } else if (SPECIAL_CLASSIFICATIONS.contains(classification)) { // is special classification
      builder.append(DELIMITER);
      builder.append(classification);
    } else if (isFGI) { // is FGI
      builder.append(DELIMITER);
      builder.append(ownerProducer);
      builder.append(SPACE);
      builder.append(classification);
