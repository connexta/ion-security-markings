 ...
 */
package com.connexta.security.markings.spring.service.impl;
...
import com.connexta.security.markings.spring.exceptions.DetailedResponseStatus;
...
import com.connexta.security.markings.util.validation.InputValidator;
...
public class ValidationServiceImpl implements ValidationService {
  ...
  ValidationServiceImpl(
    ...
    if (StringUtils.isEmpty(systemHighBannerMarkings)) {
  ...
  public void validate(SecurityMarkings securityMarkings) {
    ...
    try {
      ...
      throw new DetailedResponseStatusException(HttpStatus.UNPROCESSABLE_ENTITY, HttpStatus.UNPROCESSABLE_ENTITY.value(), e.getMessage(), e);
    }

    log.trace("Create Classification from given Security Markings.");
    Optional<Classification> optionalClassification;
    try {
      optionalClassification = classificationFactory.create(securityMarkings.getISM());
    } catch (IllegalArgumentException e) {
      throw new DetailedResponseStatusException(HttpStatus.UNPROCESSABLE_ENTITY, HttpStatus.UNPROCESSABLE_ENTITY.value(), e.getMessage(), e);
    }
